...
def expand_paginated_jsonld(url: str, paginate_cfg: Optional[Dict]) -> List[str]:
    """
    Decide which HTML pages to fetch for JSON-LD extraction:
      - if paginate_cfg has next_selector: follow that link up to max_pages
      - else: single page
    Returns a list of HTML texts.
    """
...
def main() -> int:
    ...
    for src in sources:
        ...
        try:
            if stype == "jsonld":
                ...
                for html in pages:
                    evs = extract_events_from_jsonld_html(html, url)
                    for e in evs:
                        e["_source"] = url
                    events.extend(evs)
    ...
    log(f"Wrote {OUT_ICS} and {OUT_ALIAS} with {len(vevents)} events.")
    return 0
