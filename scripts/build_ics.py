...
# === WINDOW: keep 30 days back; include 2 years ahead ===
WINDOW_START = datetime.now(timezone.utc) - timedelta(days=30)
WINDOW_END   = datetime.now(timezone.utc) + timedelta(days=730)

HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; ShropshireICSBot/1.2; +%s)" % HUB_URL}
...
def extract_events_from_jsonld(html: str, base_url: str) -> list:
    if not html: return []
...
def extract_events_from_rss(url: str) -> list:
...
def extract_events_from_ics(url: str) -> list:
...
def filter_window(evs: list, start: datetime, end: datetime) -> list:
    out = []
    for e in evs:
        sdt = parse_date_any(e.get("start","")); edt = parse_date_any(e.get("end","")) or sdt
        if not sdt: continue
        if edt < start or sdt > end: continue
...
def main() -> int:
    ...
    events = filter_window(events, WINDOW_START, WINDOW_END)
    ...
    for _, e in sorted(norm.items(), key=lambda kv: kv[1]["_sdt"]):
        ...
        # STABLE UID per event per year
        base_uid = f"{slugify(summary)}-{sdt.strftime('%Y')}@{USERNAME}.github.io"
        uid = base_uid; i = 2
        while uid in seen: uid = f"{base_uid}-{i}"; i += 1
        seen.add(uid)
        # hash for SEQUENCE
        h = hashlib.sha1("|".join([summary, sdt.isoformat(), edt.isoformat(),
                                   e["location"], e["description"], e["url"]]).encode("utf-8")).hexdigest()
        prev = state["uids"].get(uid, {})
        seq = int(prev.get("sequence", 0))
        if prev.get("hash") and prev["hash"] != h: seq += 1
        state["uids"][uid] = {"hash": h, "sequence": seq}
